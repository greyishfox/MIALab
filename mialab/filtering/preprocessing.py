...
class ImageNormalization(pymia_fltr.Filter):
    ...
    def execute(self, image: sitk.Image, params: pymia_fltr.FilterParams = None) -> sitk.Image:
        ...
        z_score = (img_arr-img_mean) / img_std
...
class ImageRegistration(pymia_fltr.Filter):
    ...
    def execute(self, image: sitk.Image, params: ImageRegistrationParameters = None) -> sitk.Image:
        ...
        if is_ground_truth:
            # here we use NearestNeighbor as interpolator because we are interested in "int" values
            # --> therefore we choose the default pixel value 0 and not 0.0 (we do not want floating point)
