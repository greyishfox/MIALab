...
def load_image(img_path, is_label_img):
    ...
    # Get and print the image dimension to check if its a 2D or 3D image (only informational part...)
    # img_width, img_height, img_depth = img.GetSize()
    # print("["+str(img_width)+", "+str(img_height)+", "+str(img_depth)+"]")

    # Casting an image's pixel type is done using the simpleITK function Cast()
    img = sitk.Cast(img, pixel_type)    # todo: modify here
